            ...
            <div class="p-5">
                ...
                @if(session('success'))
                    <div class="bg-emerald-100 border border-emerald-200 text-emerald-800 p-4 rounded-xl mb-5 text-[12px] flex items-center justify-between shadow-sm">
                        ...
                        <button onclick="this.parentElement.remove()" class="text-emerald-800 hover:text-emerald-900 font-bold ml-2 text-sm leading-none">
                            &times;
                        </button>
                    </div>
                @endif
                ...
                @if(session('error'))
                    <div class="bg-rose-100 border border-rose-200 text-rose-800 p-4 rounded-xl mb-5 text-[12px] flex items-center justify-between shadow-sm">
                        ...
                        <button onclick="this.parentElement.remove()" class="text-rose-800 hover:text-rose-900 font-bold ml-2 text-sm leading-none">
                            &times;
                        </button>
                    </div>
                @endif
                ...
                @if($data->isEmpty())
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg p-10 text-center">
                        ...
                        <p class="text-[12px] text-gray-500 mb-5">
                            Anda belum memiliki pengajuan atau pinjaman buku aktif.
                        </p>
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="flex flex-col gap-2 mt-2">
                                        <div class="flex flex-wrap gap-2">
                                            ...
                                            @else
                                                <span class="bg-gray-100 text-gray-700 px-2 py-1 rounded text-[10px] font-semibold">
                                                    {{ ucfirst($item->status) }}
                                                </span>
                                            @endif
                                        </div>
                                ...
                                @if($item->status === 'approved')
                                    <div class="flex items-center justify-end flex-shrink-0">
                                        <form method="POST" action="{{ route('borrowings.return', $item->id) }}">
                                            ...
                                            <button type="submit"
                                                    class="bg-indigo-600 hover:bg-indigo-700 text-white text-[11px] px-3.5 py-2 rounded font-semibold transition active:scale-95">
                                                Kembalikan
                                            </button>
                                        </form>
                                    </div>
